...
```{r} 
# to check and see (possible errors in data)
# unique(seqnbr) = seeing how many unique numbers there are
# since they are the same, no two id's are repeated twice

fev_tbl %>%
  summarize(num_case = length(unique(seqnbr)), num_id = length(unique(subjid)))
```
...
```{r} 
# calling it mean, but mean(age) is taking the mean of age as a function
fev_tbl %>%
...
```{r} 
# to look at only row, use filter() - filtering observations 
# use select() for columns - selecting certain columns
# == test for logical truth/a condition, whereas = assigns something to something else
# "" around words, but not around names of columns/rows
...
```{r} 
# can do filter(age >= 18 & smoke == "yes") or filter(age >= 18 | smoke == "yes") where | = or
fev_tbl %>%
  filter(age >= 18) %>%
...
```{r} 
# knows yes is a factor, so takes the mean
# can also do summarize(prop_smoke = sum(smoke=="yes")/n()) where n() is the sample size
fev_tbl %>%
  group_by(sex) %>%
...
```{r} 
# mutate creates new column in dataset
# cut function = 
fev_tbl %>% 
  mutate(age_cat = cut(age, c(0, 6, 12, 19))) %>%
...
```{r} 
# mean fev is higher for smokers than non-smokers, age might be confounding variable

fev_tbl %>%
  group_by(smoke) %>%
  summarize(mean = mean(fev), sd = sd(fev), n = n())
```
